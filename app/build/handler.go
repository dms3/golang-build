...
func commitHandler(r *http.Request) (interface{}, error) {
	...
	if dashboardForRequest(r) != gccgoDash {
		...
		if v != watcherVersion {
			return nil, fmt.Errorf("rejecting POST from commit watcher; need version %v; received version %v;",
				watcherVersion, v)
		}
	}
...
func resultHandler(r *http.Request) (interface{}, error) {
	...
	if dashboardForRequest(r) != gccgoDash {
		v, _ := strconv.Atoi(r.FormValue("version"))
		if v != builderVersion {
			return nil, fmt.Errorf("rejecting POST from builder; need version %v; received version %v;",
				builderVersion, v)
		}
	}
